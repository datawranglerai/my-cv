...
The system is built on Google's Agent Development Kit (ADK). A research coordinator delegates to pillar specialists, each a shared intelligence bus so relevant evidence artefacts from one pillar's research can be utilised by other pillars. An assessor council is organised by maturity stage rather than topic where each assessor specialises in the signals separating genuine adoption from a well-funded pilot, and the council debates the evidence over structured rounds until a consensus is reached. Multiple layers of fact-checking agents ensure the findings are honest and grounded in traceable evidence.
